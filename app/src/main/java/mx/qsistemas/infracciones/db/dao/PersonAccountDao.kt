...
interface PersonAccountDao {
    ...
    @Query("SELECT DISTINCT pt.ID_PERSONA_AYUNTAMIENTO, pt.EMPLEADO, pa.USER_NAME, pa.PASSWORD, p.NOMBRE, p.A_PATERNO, p.A_MATERNO, p.id FROM person p INNER JOIN persona_account pa ON p.id = pa.ID_PERSONA INNER JOIN person_towship pt ON p.id = pt.ID_PERSONA WHERE pa.USER_NAME = :userName")
    ...
    data class LogInUser(@ColumnInfo(name = "ID_PERSONA_AYUNTAMIENTO") var idPersonTownship: Int,
                         @ColumnInfo(name = "EMPLEADO") var employee: String,
