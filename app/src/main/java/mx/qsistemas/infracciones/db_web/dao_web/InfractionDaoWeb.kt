...
interface InfractionDaoWeb {
    ...
    suspend fun insertList(list: MutableList<InfringementInfringements>)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(infraction: InfringementInfringements): Long
    ...
    suspend fun selectInfraction(idInfraction: Long): InfringementInfringements

    @Query("SELECT folio FROM infringement_infringements WHERE folio LIKE :prefix ORDER BY id DESC LIMIT 1")
    suspend fun selectLastFolio(prefix: String): String

    @RawQuery
    suspend fun searchResumeInfraction(query: SupportSQLiteQuery): MutableList<InfractionItem>
    ...
    suspend fun updateSendByFolio(tokenServer: String, folio: String)
